...
const updatePopupShare = (blockUrl, set) => {
	if (set) {
		...
		let hash = `section=${indexActiveSection},item=${indexItem}${activeSlide ? '-' + activeSlide : ''}`
...
if (urlHash) {
	...
	const getDataItem = (objSection, indexItem) => {
		...
		if (indexItem.includes('-')) {
			arrParams.push(...indexItem.split('-'))
		} else {
			arrParams.push(indexItem)
		}

		const getItem = items => {
			items.forEach((objItem, i) => {
				if (!objItem.items) {
					if (currentIndexItem == indexItem[0]) {
						// заміна кількох реакцій на одну
						if (arrParams.length > 1) {
							objItem.content = objItem.content[arrParams[1] - 1]
						}
