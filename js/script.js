...
document.addEventListener('click', e => {
	...
	if (target.closest('.popup-reaction__fullscreen')) {
		e.preventDefault()
		const fullscreenBtn = target.closest('.popup-reaction__fullscreen')
		const templateFullscreenFormulas = '<div class="main__fullscreen fullscreen-formulas"><a href="" class="fullscreen-formulas__close"></a></div>'
		document.querySelector('.main').insertAdjacentHTML('beforeend', templateFullscreenFormulas)
		const cloneElement = fullscreenBtn.nextElementSibling.cloneNode(true)
		cloneElement.className = 'fullscreen-formulas__formulas formulas'
		document.querySelector('.fullscreen-formulas').appendChild(cloneElement)
	}

	if (target.closest('.fullscreen-formulas__close')) {
		e.preventDefault()
		target.closest('.fullscreen-formulas__close').parentElement.remove()
	}
...
function updatePopupInfo(setting) {
	...
	if (setting) {
		...
		if (Object.hasOwn(setting, 'formulas')) {
			templateBlockOpener += '<a class="popup-reaction__fullscreen" href="">Відкрити</a>' + createTemplateFormulas('popup-reaction', setting.formulas)
		}
...
if (menuListEl) {
	...
	const createTemplatePopup = (typePopup) => {
		...
		let templateFullscreenBtn = ''


		if (typePopup == 'info') {
			templateFullscreenBtn = '<a class="popup-reaction__fullscreen" href="">Відкрити</a>'
		}

		return templatePopupStart + templateOpener + templateBodyStart + templateFullscreenBtn + templateArrows + templateUrl + templateBodyEnd + templatePopupEnd
	}
...
if (urlHash) {
	...
	const getDataItem = (objSection, indexItem) => {
			...
			group.items.forEach((item, i) => {
				if (!item.items) {
					if (currentIndexItem == arrIndexs[0]) {
						const newObjItem = {}
						Object.assign(newObjItem, item)

						if (arrIndexs.length > 1) {
							console.log(arrIndexs);
							newObjItem.content = item.content[arrIndexs[1] - 1]
						}
		...
		return [objItem, objGroup.type == 'plus' ? objGroup.items[0].text : objGroup.openerText]
	}
	...
	if (objData.i) {
		...
		if (mainVideo) {
			...
			mainPlayer.landscapeFullscreen({
				fullscreen: {
					enterOnRotate: true,
