...
const updatePopupShare = (blockUrl, set) => {
	if (set) {
		...
		const activeItemMainContent = activeMainContent.querySelector('[data-tab-item].-active')

		let subject = ''
		if (activeItemMainContent.closest('[data-spollers]')) {
			subject = activeItemMainContent.closest('[data-spollers]').querySelector('[data-spoller-item]').innerText
		}

		const indexItem = Array.from(activeMainContent.querySelectorAll('[data-tab-item]')).findIndex(item => item.classList.contains('-active'))

		let hash = `section=${indexActiveSection},subject=${subject},item=${indexItem}`
...
if (menuListEl) {
	...
	const createTemplatePopup = (typePopup) => {
		...
		if (typePopup == 'switch') {
			...
			templateUrl += '<div class="popup-reaction__url">Скопіювати посилання на матеріал</div>'
		}
...
if (mainVideo) {
	...
	mainPlayer.src({ type: 'video/mp4', src: 'video/video7.mp4' });
