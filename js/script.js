...
const updatePopupShare = (blockUrl, set) => {
	if (set) {
		...
		let hash = `section=${indexActiveSection},item=${indexItem}`
...
const addMarginLock = () => {
	...
	if (window.innerWidth > bodyEl.offsetWidth) {
		...
		menuBody.style.marginLeft = -valuePaddingRight / 2 + 'px'
		bodyEl.style.paddingRight = valuePaddingRight + 'px'
	} else {
		menuBody.style.marginLeft = ''
...
if (urlHash) {
	...
	const objActiveSection = data[++objData.section]
	// console.log(objActiveSection);
	...
	if (objData.item) {
		const [objItem, textSubject] = getDataItem(objActiveSection, objData.item)
		...
		if (objFormulas) {
			...
			document.querySelector('.menu__body').classList.add('-big')
		}
