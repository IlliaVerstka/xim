...
let changeTab = (activeItem, contents, animation, oldActiveContent, state, noActive) => {
	...
	if (state == 'change') {
		...
		const visiblePopup = document.querySelector('.popup-reaction.-visible')
		if (visiblePopup) {
...
document.addEventListener('click', e => {
	...
	if (target.closest('.popup-reaction__opener')) {
		...
		const popupBody = opener.closest('.popup-reaction_info')
	...
	if (!target.closest('.popup-reaction') && document.querySelector('.popup-reaction.-open')) {
		if (document.querySelector('.popup-reaction.-open').classList.contains('popup-reaction_info')) {
			updatePopupInfo()
		}
		document.querySelector('.popup-reaction.-open').classList.remove('-open')
	}
	...
	if (type == 'video') {
		...
		if (wrapVideo.urlVideo) {
			wrapVideo.insertAdjacentHTML(
				'beforeend',
				`<video class="-reaction-video video-js"></video>`
			)
...
function updatePopupInfo(setting) {
	...
	if (setting) {
		...
		if (Object.hasOwn(setting, 'formulas')) {
			const templateFormulasStart = '<div class="popup-reaction__formulas formulas"><div class="formulas__rows">'
			const templateFormulasEnd = '</div></div>'

			// створення блоку formulas
			let templateFormulas = ''
			setting.formulas.forEach(formulaSetting => {
				// створення row
			...
			// складання формул для opener
			templateBlockOpener += templateFormulasStart + templateFormulas + templateFormulasEnd
		}
...
fetch('./json/data.min.json').then(response => response.json()).then(data => {
	...
	const tabsMainContentsEl = document.querySelector('.tabs-main__contents')
	let templateItemsMainMenu = ''
	let templateLabelsMenu = ''


	const createTemplatePopup = (typePopup) => {
		const templatePopupStart = `<div class="main-content__popup popup-reaction popup-reaction_${typePopup}">`
		const templatePopupEnd = '</div>'
		const templateOpener = '<div class="popup-reaction__opener"><a href=""><span class="-icon-arrow-right"></span></a></div>'
		...
		return templatePopupStart + templateOpener + templateBodyStart + templateArrows + templateBodyEnd + templatePopupEnd
	}
	...
	for (const indexSection in data) {
		...
		// створення попапів
		const templatePopupMainContent = createTemplatePopup('switch') + createTemplatePopup('info')

		// створення main content
		const templateItemContentsStart = '<div class="main-content__contents" data-tab-contents>'
		const templateItemContentsEnd = '</div>'
		const templateMainContentStart = '<div class="tabs-main__content main-content"><div class="main-content__body" data-tab="min, 0" data-animation-show="showTabContent, 500" data-animation-hide="hideTabContent, 500">'
